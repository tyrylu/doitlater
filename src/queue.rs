use crate::{Executable, Result, job::Job};
...
impl Queue {
    pub fn new(name: &str, redis_connection_string: &str) -> Result<Self> {
        ...
        Ok(Queue{qname: name.to_string(), redis_conn: conn, client})
    }
    ...
    pub fn enqueue(&mut self, name: &str, executable: Box<dyn Executable>) -> Result<()> {
        let pos: Option<isize> = self.redis_conn.lpos(self.immediately_executable_key(), name, LposOptions::default())?;
        if pos.is_none() {
            let job = Job::new(name, executable);
            redis::pipe().atomic().rpush(self.immediately_executable_key(), name).ignore().hset(self.jobs_storage_key(), name, job.serialize()?).ignore().query(&mut self.redis_conn)?;
        }
        Ok(())
    }

    pub fn dequeue_executable_job(&mut self, timeout: usize) -> Result<Option<Job>> {
let result: Option<(String, String)> = self.redis_conn.blpop(self.immediately_executable_key(), timeout)?;
